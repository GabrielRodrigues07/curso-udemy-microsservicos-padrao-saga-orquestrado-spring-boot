...
import br.com.microservices.choreography.productvalidationservice.config.kafka.KafkaProperties;
...
import static br.com.microservices.choreography.productvalidationservice.core.enums.ETopics.*;
...
public class SagaExecutionController {
    ...
    private void handlerSuccess(Event event) {
        ...
        String topic = kafkaProperties.getTopic().get(PAYMENT_START.getTopic());
    ...
    private void handlerFail(Event event) {
        ...
        log.info("### CURRENT SAGA: {} | SENDING TO ROLLBACK PREVIOUS SERVICE | NEXT TOPIC {} | {}",
                event.getSource(), kafkaProperties.getTopic().get(NOTIFY_ENDING.getTopic()), createSagaId(event));

        sendEvent(event, kafkaProperties.getTopic().get(NOTIFY_ENDING.getTopic()));
    }
